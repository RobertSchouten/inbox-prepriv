...
def contact_update_communication_ref(self, method=None):
	origin_communication = frappe.db.sql("select name, sender,recipients,sent_or_received from `tabCommunication`",as_dict=1)
	...
	if self.email_id:
		...
		for communication in origin_communication:
			...
			if comm.email_id:
				if (sender and communication.sent_or_received == "Received" and sender.find(comm.email_id) > -1) or (recipients and communication.sent_or_received == "Sent" and recipients.find(comm.email_id) > -1):
					if sum(1 for x in [comm.supplier, comm.customer, comm.user] if x) > 1:
		...
		for comm in origin_contact:
			if comm.email_id:
				if (doc.sender and doc.sent_or_received == "Received" and doc.sender.find(comm.email_id) > -1) or (
							doc.recipients and doc.sent_or_received == "Sent" and doc.recipients.find(comm.email_id) > -1):
					if sum(1 for x in [comm.supplier,comm.customer,comm.user] if x)>1 :
